...
> **⚠️ 场景选择决策（必读）**
>
> 根据用户意图选择正确的场景，**不要默认使用追加**：
>
> | 用户意图 | 正确场景 | 关键操作 |
> |---------|---------|---------|
> | "添加"、"追加"、"新增一节" | **场景 A**（追加） | `doc add` |
> | "在某位置插入" | **场景 B**（插入） | `doc blocks` → `doc add --block-id --index` |
> | "改一下某段文字" | **场景 C**（修改块） | `doc blocks` → `doc update` |
> | "删掉某部分" | **场景 D**（删除） | `doc blocks` → `doc delete` |
> | "编辑"、"更新"、"重写某章节"、"替换" | **场景 E**（替换）🔥 | `doc blocks` → `doc delete` → `doc add` |
>
> **常见错误**：用户说"编辑文档"或"更新某章节内容"时，错误地使用场景 A 追加，导致旧内容保留、新内容追加在末尾。
> **正确做法**：先用 `doc blocks` 定位旧内容，`doc delete` 删除，再 `doc add` 在原位置插入新内容（场景 E）。

#### 场景 A：在文档末尾追加内容

仅在用户明确要求"追加"、"新增"时使用。直接用 `doc add` 以 Markdown 格式追加：
